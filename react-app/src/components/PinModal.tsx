...
export default function PinModal({ onSubmit, onCancel, error }: PinModalProps) {
  ...
  return (
    <div className="modal-overlay active" onClick={onCancel}>
      <div className="modal-content pin-modal" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
        <div className="modal-title">🔒 {(t('pinTitle') as string) || 'Enter PIN'}</div>
        <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '12px' }}>
          {(t('pinDesc') as string) || 'Enter your PIN to access private data'}
        </p>
